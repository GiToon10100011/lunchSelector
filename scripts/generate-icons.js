...
<svg width="${size}" height="${size}" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
  ...
  <g opacity="0.6">
    ...
    fs.writeFileSync(path.join(__dirname, `../public/icon-${size}.svg`), canvas);
  });

  // 메인 아이콘 파일들도 생성
  const mainIcon = fs.readFileSync(path.join(__dirname, '../public/icon-512.svg'));
  fs.writeFileSync(path.join(__dirname, '../public/icon.png'), mainIcon);
  fs.writeFileSync(path.join(__dirname, '../public/app-icon.png'), mainIcon);
